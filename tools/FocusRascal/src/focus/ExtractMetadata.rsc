...
|file:///home/juri/Scrivania/phuong|) {
     ...
     map[str,int] dict = ();
      ...
      for(loc clazz <- clazzes){
          methodDecls = [e | e <- model.containment[clazz], e.scheme == "java+method"];
          for (loc l <- methodDecls){
              if( size(model.methodInvocation[l]) > 8){
                 ...
                 if(count > 8){
                     for (loc l1 <-  model.methodInvocation[l])
                         if(size(model.declarations[l1]) == 0)
                             appendToFile(graphName, "<l.path[1..]>#<l1.path[1..]>\n");
                 }
             }
         }
     }
...
public void jarM3(loc directory, loc outputFolder){
     for(loc l <- directory.ls) {
         createM3sFromJar(l, graphName=outputFolder);
     }
}
...
void mavenM3(bool githubDownload=true,
     ...
     loc mavenExecutable=|file:///home/juri/development/apache-maven-3.5.4/bin/mvn|,
     loc m3sPath=|project://FocusRascal/data/m3/java-projects|) {
     println(directory);
     urls = ();
     if(githubDownload) {
         urls = getGithubReposURLs(|project://FocusRascal/config/github-repos.properties|);
     ...
     for(name <- urls) {
         if(!projectExistsInLogFile(name)){
             if(!exists(m3sPath + "<toLowerCase(name)>.m3")){
                 println("Creating M3 for <name> repository <urls[name]>...");
                 repoPath = directory + name;
                 try {
                     M3 m;
                     ...
                     if(existFileWithName(repoPath, "pom.xml")) {
                         ...
                         if(!exists(m3Path)) {
                             ...
                             classPaths = getClassPath(repoPath, mavenExecutable=mavenExecutable);
                             m = createM3FromFiles(repoPath,
                                 fetchFilesByExtension(repoPath, "java"),
                                 sourcePath=[repoPath],
                                 classPath=[*classPaths[cp] | cp <- classPaths]);
                             appendToFile(m3sPath + "log.csv", "<name>,MVN,<checkTypeErrors(m)> \n");
                         }
                         else {
                             println("<name> M3 already exists. Skipping..");
                         }
                     }
                     else {
                         m = createM3FromFiles(repoPath,
                             fetchFilesByExtension(repoPath, "java"),
                             sourcePath=[repoPath]);
                         appendToFile(m3sPath + "log.csv", "<name>,PLAIN,<checkTypeErrors(m)> \n");
                     }
                     ...
                     println("Could not create M3 for <name> repository. Error: <e>");
                     continue;
                 }
             }
         }
